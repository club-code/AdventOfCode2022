...
class Day9: DaySolver(9, "Rope Bridge") {
    ...
    private inline fun Vector.inf(other: Vector) = max(abs(this.first - other.first), abs(this.second - other.second))

    private fun Vector.snapBack(other: Vector) = if (other.inf(this) <= 1) this else {
        when {
    ...
    override fun firstPart(): String {
        var head = 0 to 0
        var tail = 0 to 0
        val visited = mutableListOf<Vector>()
        visited.add(tail)
