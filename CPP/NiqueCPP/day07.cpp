...
std::optional<file*> parseLine(const std::string& s, file* cur) {
    ...
    if (split[1] == "ls") return cur;
        ...
        cur->children.push_back(new file(std::stoi(split[0]), cur, split[1]));
    } else {
        cur->children.push_back(new file(cur, split[1]));
    }
    return std::nullopt;
}
...
day07::day07() {
    ...
    root.children.push_back(new file(&root, "/"));
    file* f = &root;
...
unsigned long totalSize(const file& f) {
    if (f.val) return f.val.value();
        ...
        [](unsigned long sum, file* f){sum+=totalSize(*f); return sum;});
}
    ...
    std::function<unsigned long(const file*)> sumSmallFolders = [&sumSmallFolders](const file* f) {
        if (!f->val) {
            ...
            size += std::accumulate(f->children.begin(), f->children.end(), 0.,
                [&sumSmallFolders](unsigned long sum, file* f){sum+=sumSmallFolders(f); return sum;});
            return size;
